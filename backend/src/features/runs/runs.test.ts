...
jest.mock('../../lib/prisma', () => ({
  ...
  default: {
        ...
        describe('POST /api/runs', () => {
          ...
          it('should flag overlapping runs', async () => {
            (prisma.run.findUnique as jest.Mock).mockResolvedValue(null);
            
            // Mock that an overlapping run exists
            (prisma.run.findFirst as jest.Mock).mockResolvedValue({
              id: 'existing-run-id'
            });
        
            (prisma.run.create as jest.Mock).mockImplementation(({ data }: any) => {
            ...
            expect(response.status).toBe(201);
            expect(response.body.data.run_status).toBe('overlapping');
          });
        });
